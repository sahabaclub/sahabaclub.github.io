...
import { supabase } from "./supabase-client.js?v=90970f2d1d";
import { isMissingSchema } from "./connect.js?v=90970f2d1d";
...
// Mirrors public.profile_is_complete, which is what decides whether a row
// appears in member_directory. The database is the authority; this exists so
// the page can say what is still missing *before* the member wonders why they
// cannot find themselves.
//
// ⚠ SINCE 0062 THE ONLY REQUIREMENT IS A NAME. It used to demand a photo, a
// bio or headline and at least one interest as well, and that held 14 of 26
// members out of the directory they had opted into — every one of them on the
// interests field alone. Those three moved to `thinProfileGaps` below, which
// is a suggestion and not a gate.
//
// ⚠ KEEP THIS IN STEP WITH THE MIGRATION. If the two disagree the page tells a
// member they are listed when they are not, or the reverse, and the member has
// no way to tell which one is lying.
//
// Nothing here blocks a save: an incomplete profile saves fine.
...
// What does not stop a member being listed, but leaves them a thin card: a
// tile with a name on it and nothing to read.
//
// ⚠ ORDERED BY WHAT THE READER SEES FIRST, not by how easy each is to fill in.
// A photo is the whole card at a glance; a headline is the line under it; the
// interests are what Connect and the newsletter actually match on, so they
// come last visually and matter most for being found. All three are worth
// asking for and none is worth refusing somebody over.
export function thinProfileGaps(p) {
  const gaps = [];
  if (!has(p && p.avatar_url)) gaps.push("a photo");
  if (!has(p && p.bio) && !has(p && p.headline)) gaps.push("a short bio or headline");
  if (!(Array.isArray(p && p.interests) && p.interests.length)) gaps.push("at least one interest");
  return gaps;
}
...
// The one line under the form. Four states, none of them a failure:
// not listed and here is why, ready but switched off, listed but thin, listed.
//
// Returns `text` and `html`. They say the same thing; `html` is the one to
// render, because in it the things that can be acted on are buttons. A caller
// that only has a text node still gets a correct sentence.
export function connectHint(p) {
  ...
  // ⚠ THE NUDGE, AND IT SAYS "LISTED" FIRST ON PURPOSE. Ahmed, 10 Aug 2026,
  // once 0062 stopped these three being requirements. A member reading this is
  // already on the wall, so the sentence opens with the good news and then
  // offers one improvement — the opposite of the old line, which led with what
  // was missing and read as a refusal.
  //
  // ⚠ It names ONE thing, not three. A list of everything absent is the gate
  // again in prose, and the member who has none of the three is exactly the
  // member most likely to close the tab. `thinProfileGaps` is ordered by what
  // a reader sees first, so the head of the list is the highest-value ask.
  const gaps = thinProfileGaps(p);
  if (gaps.length) {
    const ask = gaps[0];
    const lead = "You're listed in Connect. Your card would go further with ";
    return {
      kind: "info",
      text: lead + ask + ".",
      html: escapeHtml(lead) + needHtml(ask) + ".",
    };
  }
