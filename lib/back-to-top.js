...
(function () {
  ...
  function pageIsLocked() {
    ...
    //
    // ⚠ overflowY, NOT the `overflow` shorthand. styles.css sets
    // `overflow-x: hidden` on body, so the shorthand computes to the two-value
    // form "hidden auto" — measured on the live site — and an equality test
    // against "hidden" is then comparing against a string the browser only
    // produces by coincidence when both axes happen to match. The vertical
    // axis is the one that decides whether the page scrolls.
    try {
  ...
  btn.addEventListener("click", function () {
    // ⚠ FOCUS FIRST, THEN SCROLL. THE ORDER IS THE WHOLE BUG.
    //
    // The first version scrolled and then moved focus, which reads more
    // naturally and does not work: in Chrome a programmatic focus() CANCELS an
    // in-progress smooth scroll, and `preventScroll: true` does not change that
    // — it only stops focus() from scrolling the element into view itself.
    // The button therefore travelled about 16 pixels and stopped, on every page,
    // while looking entirely correct in the source.
    //
    // Focusing first costs nothing: preventScroll means the focus call moves
    // nothing, and the scroll that follows has no interruption to survive.
    //
    // ⚠ Moving the viewport is only half the job anyway. A keyboard or
    // screen-reader user who presses this and gets no focus change is still
    // parked at the bottom of the document — the next Tab carries on from the
    // footer and the jump did nothing for them.
    //
    // tabindex is added only to make the target focusable and removed on blur,
    // so nothing permanent joins the tab order.
    ...
    if (target) {
      ...
      try {
        ...
        // Older browsers ignore the options object and scroll on focus. The
        // scroll below still lands in the right place, so this is survivable.
        target.focus();
      }
      if (!hadTabIndex) {
  ...
  // ⚠ THREE TARGETS, NOT ONE, AND THIS IS NOT SUPERSTITION.
  //
  // The first version called window.scrollTo({top:0}) alone and moved the page
  // by about 16 pixels on the live site — verified in a real browser, which is
  // the only reason it was caught at all.
  //
  // styles.css sets `overflow-x: hidden` on BOTH html and body. Setting either
  // axis to a non-visible value makes that element a scrolling box in its own
  // right, and which of html, body or the viewport actually owns the scroll
  // then depends on propagation rules that differ between engines and between
  // versions of the same engine. Measured here: `document.documentElement`
  // reports scrollTop 800 while `document.body` reports 0 — so html looks like
  // the scroller — and yet window.scrollTo moved nothing.
  //
  // Rather than pick a winner from a rule that has already been wrong once,
  // every candidate is asked. The two that are not the scroller ignore it
  // harmlessly: assigning scrollTop to a non-scrolling element is a no-op, not
  // an error.
  function jumpToTop() {
    var smooth = !motionIsReduced();
    var opts = { top: 0, left: 0, behavior: smooth ? "smooth" : "auto" };

    try {
      window.scrollTo(opts);
    } catch (e) {
      // Older engines reject the options object outright.
      try { window.scrollTo(0, 0); } catch (e2) { /* nothing else to try */ }
    }

    // The scrolling element, whichever the browser says it is, then both
    // candidates by name in case scrollingElement is the one that is wrong.
    var boxes = [document.scrollingElement, document.documentElement, document.body];
    for (var i = 0; i < boxes.length; i++) {
      var box = boxes[i];
      if (!box) continue;
      try {
        if (typeof box.scrollTo === "function") box.scrollTo(opts);
        else box.scrollTop = 0;
      } catch (e3) {
        try { box.scrollTop = 0; } catch (e4) { /* not this one */ }
      }
    }
  }
