...
// ⚠ A NAME THAT ALREADY CARRIES ITS ROUND NUMBER IS LEFT ALONE. Round 4 is
// stored as "EduHackAI-4 (Arabic Edition)", and roundFamily() only strips a
// number at the END of a name — so this appended a second one and Ahmed's
// profile read "EduHackAI-4 (Arabic Edition)-4".
function roundName(entry) {
  const name = entryText(entry.name);
  const round = entryText(entry.round);
  if (!round) return name || "EduHackAI";
  if (name && new RegExp("(^|[^0-9])" + round + "([^0-9]|$)").test(name)) return name;
  return roundFamily(entry) + "-" + round;
}
...
function roundsLabel(entries) {
  ...
  entries.forEach((e) => {
    ...
    const n = Number(raw);
    if (raw && isFinite(n)) numbered.push({ n: n, family: roundFamily(e), label: roundName(e) });
  ...
  // ⚠ COMPACTING NEEDS MORE THAN A SHARED FAMILY. "EduHackAI-1, 2, 3" is only
  // honest when every label really is the family plus its number — a round
  // named "EduHackAI-4 (Arabic Edition)" cannot be folded into that list
  // without losing the half that distinguishes it. When any label carries
  // extra text, they are all listed in full instead.
  const compactable = sorted.length &&
    sorted.every((x) => x.family === family && x.label === family + "-" + x.n);
  if (compactable) {
    parts.push(family + "-" + [...new Set(sorted.map((x) => x.n))].join(", "));
  } else {
    const seen = new Set();
    sorted.forEach((x) => {
      // x.label, not family + "-" + n — rebuilding it here is what appended
      // the second number in the first place.
      if (!seen.has(x.label)) { seen.add(x.label); parts.push(x.label); }
    });
  }
...
const CONNECT_COLUMNS =
  ...
  // ⚠ years_experience is SELECTED FOR THE CARD as of 19 Aug. It has been in
  // connect_directory all along; nothing asked for it, so `m.years_experience`
  // was undefined and the card's new "12 yrs exp" would have rendered as an
  // empty string on every row — a feature that silently does nothing, which is
  // worse than one that visibly breaks.
  "city, country, experience_level, industry, years_experience, skills, interests, links, " +
...
// ⚠ `hackathons` IS HERE BECAUSE connect_directory DOES NOT EXPOSE IT.
// That view computes over the jsonb in a lateral join to work out the relevance
// band, but never selects it — checked against information_schema, not assumed.
// The Connect card needs it for the winner's trophy, and this fetch already
// exists and is already keyed by user_id, so it carries it rather than the
// hundred-line view being rewritten to publish one more column.
const META_COLUMNS = "user_id, role, tier, profile_updated_at, open_to, hackathons";
