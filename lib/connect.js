...
import { supabase } from "./supabase-client.js?v=aba5b41535";
...
const PROFILE_COLUMNS = "work_history, hackathons, joined_site_at";
...
// What a card may say about why somebody is near the top. Every one of these
// restates a fact the reader could already have read off the other person's
// own directory row — that is the argument in 0028 for why the band is safe to
// publish, and it stops being true the moment a label here claims something
// more specific than the band it names. Bands 7 and 8 say nothing: "we found
// nothing in common" is not a caption anybody needs on a stranger.
const RELEVANCE_LABELS = {
  1: "Teammate in EduHackAI",
  2: "Same EduHackAI round",
  3: "Same round — coaching or judging",
  4: "Shared skills or interests",
  5: "Same field of work",
  6: "Same country",
};

export function relevanceLabel(band) {
  return RELEVANCE_LABELS[Number(band)] || "";
}
  ...
  const ranked = await pageAll(
    (from, to) =>
      supabase
        ...
        // ⚠ NEWEST FIRST, not best-related first. Ahmed, 10 Aug 2026: sort
        // Connect on latest joined. `relevance` was the leading key until then,
        // which meant the order changed per viewer and a member who joined this
        // morning could sit below somebody from months ago because they shared
        // a city with you.
        //
        // ⚠ `relevance` is still SELECTED and still shown — `relevanceLabel()`
        // puts it on each card ("Same city", "Same industry"). It is a label
        // per person, not a grouping, so dropping it from the ORDER changes
        // where a card sits and nothing about what it says. Do not remove it
        // from CONNECT_COLUMNS on the assumption it is now unused.
        ...
        // `listed_since` is the leading key, and its biggest tie is the whole
        // prospect import.
