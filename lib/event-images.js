...
export const IMAGE_SPECS = {
  ...
  // 0078 — photographs from a past Hub event. Deliberately the most forgiving
  // spec of the three: these are pictures somebody took on the day, on whatever
  // they had in their pocket, and there is no reshaping them afterwards.
  eventGallery: {
    key: "eventGallery",
    label: "Event photo",
    // The grid renders them square-cropped at ~140–200 CSS px, and clicking
    // one opens the original. 1600 on the long edge is plenty for that and
    // still inside the bucket's 5 MB ceiling for a normal photograph.
    recommended: [1600, 1200],
    min: [640, 480],
    aspectHint: "any shape — the grid crops to a square, the full picture opens on click",
    // ⚠ Ratio effectively unchecked. A phone portrait, a landscape from a
    // camera and a screenshot are all legitimate here; warning about the shape
    // of a photograph nobody can retake is noise the uploader cannot act on.
    ratio: 1,
    ratioTolerance: 5,
    why:
      "Shown in the gallery grid on the event page, cropped to a square " +
      "thumbnail, and opened full-size when somebody clicks it. Up to 20 per " +
      "event. Anything reasonably sharp works — these are photographs of the " +
      "day, not artwork.",
  },
};
